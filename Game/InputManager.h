...
void scroll_callback(GLFWwindow* window, double xoffset, double yoffset)
{
	...
	// Adjust the camera position based on yoffset.
	// You might want to scale yoffset by a certain factor to control the speed.
	float zoomAmount = (float)yoffset; // You can adjust this scaling factor as needed.
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	if (action == GLFW_PRESS || action == GLFW_REPEAT)
	{
		...
		case GLFW_KEY_UP:
			...
			scn->MyRotate(2.0f, glm::vec3(-1, 0, 0), 0);
			break;
		case GLFW_KEY_DOWN:
			scn->MyRotate(2.0f, glm::vec3(1, 0, 0), 0);
			//scn->MoveCamera(0, scn->zTranslate, -0.4f);
		...
		case GLFW_KEY_LEFT:
			//scn->MoveCamera(0, scn->zTranslate, 0.4f);
			scn->MyRotate(2.0f, glm::vec3(0, -1, 0),0);
			break;
		case GLFW_KEY_RIGHT:
			//scn->MoveCamera(0, scn->zTranslate, -0.4f);
			scn->MyRotate(2.0f, glm::vec3(0, 1, 0), 0);
...
>>>>>>> a0b4d5e43bedb3b580e412a43f81aa9d55a8c824
